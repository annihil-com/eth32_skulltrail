...
public:
	...
	void MouseEvent(UINT mevent);			// mouse event from wndproc
	void MouseWheelEvent(UINT status);		// mouse wheel from wndproc
	...
	void SetWindowHandle(HWND hWnd);		// set our game window handle from CreateWindowEx
	void RedirectWndProc(void);				// redirect game window winproc to our own to get input msgs from system
	void RestoreWndProc(void);				// restore winproc to original
	
	CWindow *CreateWin(char *title, int x, int y, int w, int h, wintype_t = WIN_STANDARD, viewtype_t vtype = GUI_MAINVIEW);	// create a window
	...
	void IrcMessage(const char *nick, const char *msg);
	void IrcStatusMessage(const char *msg);
...
private:
	...
	CIrcWindow		*winIrcChat;			// allow access to irc window functions
	CRadarWindow	*winRadar;				// allow access to radar functions
	
	HWND		hwnd;						// handle to game window
	...
	CRITICAL_SECTION	*_csCenterPrint;
	CRITICAL_SECTION	*_csConsoleCmds;
	CRITICAL_SECTION	*_csConsoleMsgs;
};
...
LRESULT __stdcall eth32_WndProc( HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam );