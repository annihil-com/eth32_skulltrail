...
void CIntSlider::Display(void)
{
	...
	char buf[16];
	sprintf_s(buf,sizeof(buf),"%i", *target);
...
int CIntSlider::ProcessMouse(int mx, int my, UINT event, CControl **mhook, CControl **khook)
{
	if (!target)
		return 1;
...
void CFloatSlider::Display(void)
{
	...
	char buf[16];
	sprintf_s(buf,sizeof(buf),"%.3f", *target);
...
int CFloatSlider::ProcessMouse(int mx, int my, UINT event, CControl **mhook, CControl **khook)
{
	if (!target)
		return 1;
...
int CDropbox::ProcessMouse(int mx, int my, UINT mevent, CControl **mhook, CControl **khook)
{
	if (!target)
		return 1;
