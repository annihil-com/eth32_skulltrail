...
CControl::CControl(const char *clabel, int cx, int cy, int cw, int ch) // x,y,w,h in 640x480 for layout purposes, convert to real res
{
	...
	strcpy_s(label, (strlen(clabel)+1), clabel);
...
{
	strncpy_s(s,s_s, label, _TRUNCATE);
	return s;
}
...
int CControl::ProcessMouse(int mx, int my, UINT mevent, CControl **mhook, CControl **khook)
{
	if (INBOUNDS(mx,my))
		return 0;
	else
...
void CControl::SetLabel(const char *newLabel)
{
	if (newLabel && *newLabel) {
		...
		strcpy_s(label, strlen(newLabel)+1, newLabel);
	}
}
...
{
	char txt[16];
	sprintf_s(txt,sizeof(txt), "%i", *target);
...
{
	char txt[16];
	sprintf_s(txt,sizeof(txt), "%.2f", *target);
