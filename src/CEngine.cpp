...
void CEngine::RegisterMedia()
{
	...
	eth32.cg.media.hitlerBodySkin = orig_syscall(CG_R_REGISTERSKIN, "models/hitler_body.skin");
...
void CEngine::PostFrame(void)
{
	...
	for (int i=0 ; i<numFramePlayers ; i++) {
		...
		if (eth32.settings.drawBodyHitbox)
			Aimbot.DrawBodyBox( player->clientNum );
	...
	if (!Gui.IsLimboPanelUp() && eth32.cg.snap->ps.pm_type != PM_INTERMISSION) {
...
int CEngine::CG_DrawActiveFrame(int serverTime, int stereoView, int demoPlayback)
{
	...
	eth32.cg.snap->ps.powerups[PW_BLACKOUT] = 0; //Speclock removal
...
void CEngine::AddRefEntity(refEntity_t *re)
{
		...
		)
	{
		...
		if(re->hModel == eth32.cg.media.medpackTCE) {
			re->customShader = eth32.cg.media.Shader1[eth32.settings.itemShader1];
	...
	if(re->hModel == eth32.cg.media.thompsonModel || re->hModel == eth32.cg.media.mp40Model || re->hModel == eth32.cg.media.stenModel)
	{
		...
		if (eth32.settings.itemShader1Wallhack && eth32.settings.wallhack)
			re->renderfx |= RF_DEPTHHACK;	// Wallhack for both shaders

		if (eth32.settings.itemShader1) {
			re->customShader = eth32.cg.media.Shader3[eth32.settings.itemShader1];
			memcpy(re->shaderRGBA, eth32.settings.colorWorldWeapons, sizeof(BYTE[3]));
			re->shaderRGBA[3] = 0xff;	// 255
		}
		
		if (eth32.settings.itemShader2Wallhack && eth32.settings.wallhack) {
			Syscall.R_AddRefEntityToScene(re);
			re->renderfx |= RF_DEPTHHACK;	// We need this in case shader1 wallhack is off
		} else {
			Syscall.R_AddRefEntityToScene(re);
			re->renderfx &= ~RF_DEPTHHACK; 	// Skip shader2 wallhack
		}
		
		if (eth32.settings.itemShader2) {
			re->customShader = eth32.cg.media.Shader4[eth32.settings.itemShader2];
			memcpy(re->shaderRGBA, eth32.settings.colorWorldWeaponsOutline, sizeof(BYTE[3]));
	...
	if(re->hModel == eth32.cg.media.medPack || re->hModel == eth32.cg.media.ammoPack)
	{
		...
		if (eth32.settings.itemShader2) {
			re->customShader = eth32.cg.media.Shader4[eth32.settings.itemShader2];
			memcpy(re->shaderRGBA, (re->hModel == eth32.cg.media.medPack) ? eth32.settings.colorHealthOutline : eth32.settings.colorAmmoOutline, sizeof(BYTE[3]));
			re->shaderRGBA[3] = 0xff;
		}
	}	
	...
	if (pDrawPlayer && (re->reType != RT_SPRITE)) { // this is part of a player and isn't an icon above it's head
		if (re->hModel == pDrawPlayer->headModel) {
			...
			if (eth32.settings.reaperHead) {
				re->hModel = eth32.cg.media.reaperHead;
				re->customShader = eth32.cg.media.reaperSkin;
			} else if (eth32.settings.hitlerHead) {
				re->hModel = eth32.cg.media.hitlerHead;
				re->customSkin = eth32.cg.media.hitlerHeadSkin;
			}
		}
		
		// Custom Body model
		if (eth32.settings.hitlerBody && re->torsoFrameModel) {			
			re->hModel = eth32.cg.media.hitlerBody;
			re->customSkin = eth32.cg.media.hitlerBodySkin;
		}
		...
		if (drawWeapon) {
			...
			if (eth32.settings.weaponShader1) {
				re->customShader = eth32.cg.media.Shader3[eth32.settings.weaponShader1];
			...
			if (eth32.settings.weaponShader2) {
                    		re->customShader = eth32.cg.media.Shader4[eth32.settings.weaponShader2];
         			memcpy(re->shaderRGBA, (pDrawPlayer->friendly) ? eth32.settings.TeamWeaponOutline : eth32.settings.EnemyWeaponOutline, sizeof(BYTE[3]));
				re->shaderRGBA[3] = 0xff;
			}	
		...
		// Player chams 
			...
			if (pDrawPlayer->friendly) {
				...
				if (eth32.settings.teamShader2) {
					re->customShader = eth32.cg.media.Shader2[eth32.settings.teamShader2];
					memcpy(re->shaderRGBA, eth32.settings.colorTeamOut, sizeof(BYTE[3]));
					re->shaderRGBA[3] = 0xff;
				}
			} else {
				...
				if (eth32.settings.enemyShader1) {
					re->customShader = eth32.cg.media.Shader1[eth32.settings.enemyShader1];
					if (pDrawPlayer->invulnerable) 
						memcpy(re->shaderRGBA, eth32.settings.colorInvulnerable, sizeof(BYTE[3]));
					else
				...
				if (eth32.settings.enemyShader2) {
					re->customShader = eth32.cg.media.Shader2[eth32.settings.enemyShader2];
					memcpy(re->shaderRGBA, eth32.settings.colorEnemyOut, sizeof(BYTE[3]));			
					re->shaderRGBA[3] = 0xff;
				}
			}
		}
 	...
 	// Own Weapon chams
		...
		if (eth32.settings.weaponShader2Wallhack && eth32.settings.wallhack) {
		    ...
		    re->renderfx &= ~RF_DEPTHHACK; 	// Skip shader2 wallhack
     		...
     		if (eth32.settings.weaponShader2) {
     			re->customShader = eth32.cg.media.Shader4[eth32.settings.weaponShader2];
       			memcpy(re->shaderRGBA, eth32.settings.TeamWeaponOutline, sizeof(BYTE[3]));
			re->shaderRGBA[3] = 0xff;
		}
	}
...
void CEngine::PlayerEsp(player_t *player)
{
	...
	if (Draw.worldToScreen(espOrigin, &player->screenX, &player->screenY)) {
		...
		if (eth32.settings.classEspType) {
			float iconSize, x, y; 		
			vec4_t iconColor;
			player->friendly ? VectorCopy(eth32.settings.clsTeam, iconColor) : VectorCopy(eth32.settings.clsEnemy, iconColor);
			iconColor[3] = eth32.settings.clsOpacity;
		if (eth32.settings.classEspType == CLS_OFF) {
			} else if (eth32.settings.classEspType == CLS_STATIC) {
			iconSize = eth32.settings.clsSize;
				...
				y -= iconSize + 2;
 				...
 				Syscall.R_SetColor(NULL);
			} else if (eth32.settings.classEspType == CLS_DIST) {
